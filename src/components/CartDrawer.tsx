      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            tabIndex={-1}
            className="fixed inset-y-0 right-0 z-[71] flex w-full max-w-md flex-col bg-bone text-ink outline-none"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
          ...
          >
            <div className="flex items-center justify-between border-b border-ink/15 px-6 py-5">
              <div className="flex items-center gap-3">
                ...
                <h2 className={`${type.h3} font-medium uppercase`}>{STEP_TITLES[step]}</h2>
              </div>
              <button
                type="button"
                onClick={requestClose}
            ...
            {step === "success" && paymentId ? (
              ...
              <>
                  ...
                  {lines.map((line) => (
                    ...
                    >
                      <div className="flex flex-col gap-1">
                        ...
                        <span className={`text-xs text-ink/50 ${type.mono}`}>
                          {formatINR(line.unitPrice)} each
                        </span>
                        <button
                          type="button"
                          onClick={() => removeItem(line.variantId)}
                          className="mt-1 flex items-center gap-1.5 text-xs text-ink/50 hover:text-ink"
                        >
                ...
                <div className="flex flex-col gap-4 border-t border-ink/15 px-6 py-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs tracking-[0.2em] text-ink/50 uppercase">Subtotal</span>
